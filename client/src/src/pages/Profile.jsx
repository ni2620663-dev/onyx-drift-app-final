      ...
      <div className="max-w-4xl mx-auto px-4 -mt-24 relative z-20 pb-20">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-3 mt-4">
                ...
                <span className="px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-zinc-400 text-[10px] font-bold uppercase tracking-widest">
                  {userProfile?.drifterLevel || "Elite Drifter"}
                </span>
              </div>
            </div>
        ...
        <div className="min-h-[400px]">
          <AnimatePresence mode="wait">
            ...
            {activeTab === "Assets" && (
              <motion.div key="assets" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {userProfile?.unlockedAssets?.map((asset) => (
                  <div key={asset} className="p-6 bg-white/5 border border-white/10 rounded-[2.5rem] flex justify-between items-center">
                    <div className="flex items-center gap-4">
                      ...
                      <p className="text-white font-black uppercase text-[10px]">{asset.replace('_', ' ')}</p>
                    </div>
                    <button onClick={() => handleEquipAsset(asset, 'aura')} className="px-5 py-2 rounded-xl text-[9px] bg-white/5 text-zinc-400 hover:bg-cyan-500 hover:text-black uppercase font-black">Equip</button>
                  </div>
                ))}
              </motion.div>
            )}
