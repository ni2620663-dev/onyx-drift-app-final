      ...
      <div className={`flex flex-col h-full w-full ${currentChat ? 'hidden md:flex' : 'flex'}`}>
        <header className="p-6 flex justify-between items-center bg-black/20 border-b border-white/5 backdrop-blur-xl">
          <div className="flex items-center gap-4">
            <img src={user?.picture} className="w-11 h-11 rounded-full border-2 border-cyan-500/30" alt="" />
            <h1 className="text-2xl font-black italic text-cyan-500 uppercase tracking-tighter">OnyxDrift</h1>
          </div>
          <button onClick={() => setIsGroupModalOpen(true)} className="p-3 bg-zinc-900 rounded-2xl text-cyan-500 border border-white/10 active:scale-95 transition-all">
            <HiPlus size={24}/>
          </button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto pb-28 no-scrollbar px-4 space-y-2 mt-4">
          {conversations.map(c => (
            <div key={c._id} onClick={() => setCurrentChat(c)} className="p-4 flex items-center gap-4 hover:bg-zinc-900/60 rounded-[2rem] cursor-pointer transition-all border border-transparent hover:border-white/5">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-center">
                      <span className="font-bold">{c.isGroup ? c.groupName : (c.userDetails?.name || "Neural Link")}</span>
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] h-20 bg-[#111]/80 backdrop-blur-3xl rounded-[2.5rem] border border-white/10 flex justify-around items-center">
        ...
        <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed inset-0 bg-[#050505] z-[200] flex flex-col">
           <header className="p-4 flex justify-between items-center border-b border-white/5 bg-black/60 backdrop-blur-xl">
              <div className="flex items-center gap-3">
                 ...
                 <h3 className="text-[15px] font-bold">{currentChat.isGroup ? currentChat.groupName : (currentChat.userDetails?.name || "Unknown")}</h3>
              </div>
