...
import GroupMessenger from "./GroupMessenger"; // New Component
...
const Messenger = ({ socket }) => {
  ...
  const getAvatar = (u) => u?.avatar || u?.picture || `https://ui-avatars.com/api/?name=${getDisplayName(u)}`;
      ...
      <div className={`flex flex-col h-full w-full ${currentChat ? 'hidden' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 custom-scrollbar">
          {activeTab === "chats" ? (
            ...
            <GroupMessenger socket={socket} API_URL={API_URL} getAuthToken={getAuthToken} />
          )}
        </div>
        ...
        <nav className="p-4 pb-8 flex justify-around items-center bg-black/80 backdrop-blur-2xl border-t border-white/5 shrink-0">
          <button onClick={() => setActiveTab("chats")} className={`p-3 flex flex-col items-center gap-1 ${activeTab === "chats" ? 'text-cyan-500' : 'text-zinc-600'}`}>
            ...
            <span className="text-[8px] font-black uppercase tracking-tighter">Chats</span>
          </button>
          <button onClick={() => setActiveTab("groups")} className={`p-3 flex flex-col items-center gap-1 ${activeTab === "groups" ? 'text-cyan-500' : 'text-zinc-600'}`}>
            <HiUsers size={24} />
            <span className="text-[8px] font-black uppercase tracking-tighter">Groups</span>
          </button>
          <button onClick={() => setActiveTab("settings")} className={`p-3 flex flex-col items-center gap-1 ${activeTab === "settings" ? 'text-cyan-500' : 'text-zinc-600'}`}>
            <HiCog6Tooth size={24} />
            <span className="text-[8px] font-black uppercase tracking-tighter">Drift</span>
          </button>
        </nav>
      </div>
          ...
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25 }} className="fixed inset-0 z-[200] flex flex-col h-[100dvh] bg-[#02040a]">
            <header className="p-3 pt-10 flex justify-between items-center border-b border-white/5 bg-black/80 backdrop-blur-xl shrink-0">
               ...
               <div className="flex gap-2">
                 <button className="text-zinc-500 p-2"><FaPhone size={18}/></button>
