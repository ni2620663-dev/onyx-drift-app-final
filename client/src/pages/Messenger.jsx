...
const Messenger = ({ socket }) => {
  ...
  /* =================🛠 HELPERS (User Name Fix) ================= */
  
  const getDisplayName = (u) => {
    if (!u) return "Unknown Drifter";
    // নাম পাওয়ার অগ্রাধিকার: name -> nickname -> displayName -> Auth0ID Slice
    return u.name || u.nickname || u.displayName || (u.auth0Id ? `Drifter_${u.auth0Id.slice(-4)}` : "Drifter");
  };

  const getAvatar = (u) => {
    return u?.avatar || u?.picture || `https://ui-avatars.com/api/?name=${encodeURIComponent(getDisplayName(u))}&background=0D8ABC&color=fff&bold=true`;
  };
  ...
  const getAuthToken = useCallback(async () => {
    try {
      ...
      console.error("Neural Token Error", e);
      return null;
    }
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      // রুটটি /api/users এ ফিক্স করা হয়েছে আপনার ব্যাকএন্ড অনুযায়ী
      const res = await axios.get(`${API_URL}/api/users/search-users/${query}`, {
  ...
  const startNewConversation = async (selectedUser) => {
    try {
      ...
      console.error("Establish Connection Error", err);
    }
  };
  ...
  const fetchConversations = useCallback(async () => {
    ...
    } catch (err) { console.error("Neural Sync Error:", err); }
  }, [getAuthToken]);
  ...
  const fetchMessages = async (convId) => {
    ...
    } catch (err) { console.error("Neural Buffer Error:", err); }
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.error("Neural Transmission Failed:", err.message); 
    }
  };
      ...
      <AnimatePresence>
        ...
        {showNotification && (
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed inset-0 z-[300] bg-[#02040a] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4"><Notification /></div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <header className="p-5 pt-12 flex flex-col gap-4 bg-black/40 border-b border-white/5 backdrop-blur-3xl shrink-0">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[8px] text-zinc-500 font-bold uppercase tracking-widest">{getDisplayName(user)} • Network Active</p>
              </div>
            </div>
          ...
          {activeTab !== "settings" && (
            <div className="relative group">
              ...
              <input type="text" value={searchQuery} onChange={(e) => handleSearch(e.target.value)} placeholder="Search Drifters..." className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-sm outline-none focus:border-cyan-500/50" />
              <AnimatePresence>
                {searchQuery.length > 0 && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute top-full left-0 right-0 mt-2 bg-[#0d1117] border border-white/10 rounded-2xl shadow-2xl z-[50] max-h-60 overflow-y-auto">
                    {isSearching ? <div className="p-4 text-center text-xs text-zinc-500">Scanning Grid...</div> : 
                      searchResults.map(u => (
