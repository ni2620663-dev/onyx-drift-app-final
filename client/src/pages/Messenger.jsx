...
import { 
  ...
  HiOutlinePaperAirplane, HiUserGroup, HiShieldCheck, HiLightningBolt, HiSparkles
    ...
    <div className="fixed inset-0 bg-[#050505] text-white font-sans overflow-hidden z-[9999]">
      ...
      <div className={`flex flex-col h-full w-full ${currentChat ? 'hidden md:flex' : 'flex'}`}>
        <header className="p-6 flex justify-between items-center bg-black/40 border-b border-white/5 backdrop-blur-2xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-500 font-bold tracking-widest uppercase">Fast. Secure. Smart.</p>
            </div>
          </div>
          <button className="p-3 bg-zinc-900/50 rounded-2xl text-cyan-500 border border-white/10 active:scale-90 transition-all">
            <HiPlus size={24}/>
          </button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto pb-32 no-scrollbar px-4 mt-4">
          ...
          {/* 🔹 PREMIUM BIO CARD (একসাথে করা বায়ো সেকশন) */}
          <motion.div 
          ...
          >
            ...
            <h2 className="text-sm font-black text-cyan-500 mb-2 uppercase tracking-widest flex items-center gap-2">
              <HiLightningBolt /> Next-Gen Messenger
            </h2>
            
            <p className="text-[13px] text-gray-300 leading-relaxed mb-4">
              দ্রুত, নিরাপদ আর স্মার্ট মেসেঞ্জার। Chat, call আর privacy সবকিছু একসাথে। <br/>
              <span className="text-zinc-500 italic">No ads. No tracking. Just pure connection.</span>
            </p>

            <div className="grid grid-cols-3 gap-2">
              <div className="bg-white/5 p-2 rounded-2xl border border-white/5 text-center">
                <HiLightningBolt className="mx-auto text-cyan-500 mb-1" size={16}/>
                <span className="text-[9px] font-black uppercase text-gray-400">Fast</span>
              </div>
              <div className="bg-white/5 p-2 rounded-2xl border border-white/5 text-center">
                <HiShieldCheck className="mx-auto text-cyan-500 mb-1" size={16}/>
                <span className="text-[9px] font-black uppercase text-gray-400">Private</span>
              </div>
              <div className="bg-white/5 p-2 rounded-2xl border border-white/5 text-center">
                ...
                <div key={c._id} onClick={() => setCurrentChat(c)} className="p-4 flex items-center gap-4 hover:bg-zinc-900/60 rounded-[2rem] cursor-pointer transition-all border border-transparent hover:border-white/5 group">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-center">
                          <span className="font-bold text-gray-100">{displayName}</span>
                          <span className="text-[10px] text-cyan-500 font-black">STABLE</span>
                      </div>
                      <p className="text-[12px] text-zinc-500 truncate">{c.lastMessage || "Encrypted signal active..."}</p>
                    </div>
                </div>
              );
            })}
        ...
        {/* --- Navigation Bar --- */}
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] h-20 bg-[#111]/90 backdrop-blur-3xl rounded-[2.5rem] border border-white/10 flex justify-around items-center z-[110] shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
      ...
      <AnimatePresence>
      {currentChat && (
        <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25 }} className="fixed inset-0 bg-[#050505] z-[200] flex flex-col">
           <header className="p-4 flex justify-between items-center border-b border-white/5 bg-black/60 backdrop-blur-xl">
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    <h3 className="text-[15px] font-bold">{currentChat.isGroup ? currentChat.groupName : (currentChat.userDetails?.name || "Drifter")}</h3>
                    <p className="text-[9px] text-cyan-500 uppercase font-black tracking-widest">End-to-End Secure</p>
                 </div>
              </div>
              <div className="flex gap-2">
                 <button className="p-3 text-cyan-500 bg-cyan-500/10 rounded-2xl"><HiOutlineVideoCamera size={24}/></button>
           ...
           <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
              {messages.map((m, i) => (
                <div key={i} className={`flex flex-col ${m.senderId === user?.sub ? 'items-end' : 'items-start'}`}>
                  <div className={`px-5 py-3 rounded-[1.8rem] max-w-[85%] ${m.senderId === user?.sub ? 'bg-cyan-600 text-white rounded-tr-none shadow-lg shadow-cyan-900/20' : 'bg-zinc-900 text-zinc-100 rounded-tl-none border border-white/5'}`}>
           ...
           <div className="p-4 pb-10 bg-black/60 flex items-center gap-3">
              <div className="flex-1 flex items-center gap-3 bg-[#111] p-2 rounded-[2.5rem] border border-white/10">
                 <input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSend()} placeholder="Type a message..." className="bg-transparent flex-1 px-4 outline-none text-white text-sm" />
                 <button onClick={handleSend} className="p-3.5 bg-cyan-500 text-black rounded-full shadow-lg active:scale-90 transition-all">
