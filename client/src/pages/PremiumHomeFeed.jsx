  ...
  return (
    <div className="w-full min-h-screen bg-black text-white pb-32 font-sans overflow-x-hidden selection:bg-cyan-500/30">
      ...
      <motion.button 
        ...
        className="fixed bottom-8 right-6 w-16 h-16 bg-cyan-500 rounded-2xl flex items-center justify-center shadow-[0_0_30px_rgba(6,182,212,0.5)] z-[1000] group"
      >
        <FaBolt className="text-black text-xl group-hover:animate-bounce" />
        <div className="absolute inset-0 rounded-2xl border-2 border-cyan-400 animate-pulse" />
      </motion.button>

      {/* BOTTOM NAV (MOBILE) */}
      <nav className="fixed bottom-0 left-0 right-0 h-20 bg-black/80 backdrop-blur-2xl border-t border-white/5 flex justify-around items-center px-6 z-[1000]">
        {[
          { icon: <FaHome />, tab: 'home' },
          { icon: <FaSearch />, path: '/search' },
          { icon: <FaStore />, tab: 'market' },
          { icon: <FaRegBell />, tab: 'notify' }
        ].map((item, i) => (
          <button 
            key={i} 
            onClick={() => {
            ...
            className={`text-xl p-3 rounded-2xl transition-all ${activeTab === item.tab ? 'text-cyan-500 bg-cyan-500/10 shadow-[0_0_20px_rgba(6,182,212,0.1)]' : 'text-zinc-600 hover:text-zinc-400'}`}
          >
            {item.icon}
          </button>
        ))}
