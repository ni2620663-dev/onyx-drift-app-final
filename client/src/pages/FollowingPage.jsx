...
const API_URL = "https://onyx-drift-app-final-u29m.onrender.com";
  ...
  const loadProfileData = useCallback(async (id) => {
    try {
      ...
      // প্রোফাইল ও পোস্ট প্যারালাল ফেচ (Performance Optimization)
  ...
  /**
   * ২. ইউজার সার্চ বা ডিসকভারি লিস্ট
   */
  const loadDiscoveryList = useCallback(async (query = "") => {
    try {
      ...
      setUsers(res.data || []);
      setPosts([]); // ডিসকভারি মোডে পোস্ট ক্লিয়ার রাখা
    } catch (err) {
  ...
  useEffect(() => {
    if (targetUserId) {
      ...
      }, 500); // সার্চের জন্য ৫০০ms ডিবউন্স
    ...
    <div className="p-4 md:p-6 bg-transparent min-h-screen font-sans max-w-7xl mx-auto selection:bg-cyan-500/30">
        ...
        {!targetUserId && (
          <div className="relative w-full md:w-96">
            <input 
              ...
              className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-white text-xs outline-none focus:border-cyan-500/50 transition-all backdrop-blur-xl"
              value={searchTerm}
          ...
          >
            <div className="flex flex-col items-center text-center">
              <div className="relative cursor-pointer" onClick={() => navigate(`/following?userId=${u.auth0Id}`)}>
                <img 
                  src={u.avatar || u.picture || `https://ui-avatars.com/api/?name=${u.name}`} 
              ...
              <h3 className="text-white font-black text-xl mt-4 italic uppercase">{u.name}</h3>
              <p className="text-cyan-400/40 text-[9px] font-black tracking-widest uppercase">@{u.nickname || "drifter"}</p>
            </div>
            
            <div className="mt-8 grid grid-cols-3 gap-3">
                <button onClick={() => handleFollow(u.auth0Id)} className="flex flex-col items-center p-3 bg-white/5 rounded-2xl text-cyan-500 hover:bg-cyan-500 hover:text-black transition-all">
                  <FaUserPlus size={16} />
                  <span className="text-[7px] font-black mt-1 uppercase">Follow</span>
                </button>
                <button onClick={() => navigate(`/messenger?userId=${u.auth0Id}`)} className="flex flex-col items-center p-3 bg-white/5 rounded-2xl text-purple-500 hover:bg-purple-600 hover:text-white transition-all">
                  <FaEnvelope size={16} />
                  <span className="text-[7px] font-black mt-1 uppercase">Chat</span>
                </button>
                <button onClick={() => navigate(`/call/${u.auth0Id}`)} className="flex flex-col items-center p-3 bg-white/5 rounded-2xl text-emerald-500 hover:bg-emerald-600 hover:text-white transition-all">
                  <FaPhoneAlt size={16} />
                  <span className="text-[7px] font-black mt-1 uppercase">Call</span>
                </button>
            </div>
          </div>
        )) : !loading && <p className="text-gray-500 text-center col-span-full">No Drifter Found.</p>}
      </div>
      ...
      {targetUserId && !loading && (
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {posts.map(post => (
              <div key={post._id} className="bg-white/5 border border-white/10 p-5 rounded-[2rem]">
                {post.media && (
                ...
                <p className="text-gray-300 text-sm">{post.text || post.content}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {loading && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-[100]">
          <div className="w-10 h-10 border-4 border-t-cyan-500 border-cyan-500/20 rounded-full animate-spin"></div>
        </div>
      )}
    </div>
  );
